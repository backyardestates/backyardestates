import { sanityFetch } from '@/sanity/live'
import { defineQuery } from 'next-sanity'
import { notFound } from 'next/navigation'
import { PortableText } from 'next-sanity'
import type { Metadata } from 'next'
import { buildMetadata } from '@/lib/seo'
import JsonLd from '@/components/JsonLd'
import { articleSchema } from '@/lib/jsonLd'

type Props = {
    params: Promise<{ category: string; post: string }>
}

const CLOUDINARY = 'https://res.cloudinary.com/backyardestates/image/upload'

export async function generateMetadata({ params }: Props): Promise<Metadata> {
    const { post, category } = await params

    const { data: postObj } = await sanityFetch({
        query: POST_QUERY,
        params: { post },
    })

    const image = postObj?.image?.public_id
        ? `${CLOUDINARY}/${postObj.image.public_id}`
        : undefined

    return buildMetadata({
        title: `${postObj?.title} - Blog`,
        description:
            postObj?.description ||
            `${postObj?.title} — an ADU guide from Backyard Estates, builders across the Inland Empire and Los Angeles area.`,
        path: `/blog/${category}/${post}`,
        image,
        type: 'article',
    })
}

import Footer from '@/components/Footer'
import Nav from '@/components/Nav'

import style from '../../blog.module.css'
import Breadcrumbs from '@/components/Breadcrumbs'
import Catchall from '@/components/AttentionCTA'
import BlogCategory from '@/components/BlogCategory'
import BlogTag from '@/components/BlogTag'
import BlogAuthor from '@/components/BlogAuthor'
import BlogImage from '@/components/BlogImage'
import AttentionCTA from '@/components/AttentionCTA'

const POST_QUERY = defineQuery(`*[
	_type == "post" &&
	slug.current == $post
  ][0]{title, image, author->{firstname, lastname, portrait}, _updatedAt, description, content, slug, categories->{title, slug}, tags[]->{title, slug}}`)

export default async function Post({
    params,
}: {
    params: Promise<{ slug: string }>
}) {
    const { data: post } = await sanityFetch({
        query: POST_QUERY,
        params: await params,
    })

    if (!post) {
        notFound()
    }

    const pages = [
        { title: 'Blog', href: '/blog' },
        {
            title: post.categories.title,
            href: `/blog/${post.categories.slug.current}`,
        },
    ]

    const tags = post.tags

    const articleLd = articleSchema({
        title: post.title,
        slug: post.slug?.current,
        category: post.categories?.slug?.current,
        image: post.image?.public_id
            ? `${CLOUDINARY}/${post.image.public_id}`
            : undefined,
        updatedAt: post._updatedAt,
        publishedAt: post._updatedAt,
    })

    return (
        <>
            <JsonLd data={articleLd} />
            <Nav />
            <main className={`${style.main} ${style.mainPadded}`}>
                <Breadcrumbs pages={pages} />
                <div className={style.content}>
                    <BlogCategory category={post.categories.title} />
                    <h1>{post.title}</h1>
                    <BlogAuthor author={post.author} date={post._updatedAt} />
                    <BlogImage public_id={post.image.public_id} alt={post.title} />
                    <p className={style.description}>{post.description}</p>
                    <PortableText value={post.content} />
                    <h3>Blog post tags</h3>
                    <div className={style.tags}>
                        {tags.map((tag, index) => (
                            <BlogTag key={index} tag={tag} />
                        ))}
                    </div>
                </div>
                <AttentionCTA
                    eyebrow="Get Started"
                    title="Start your ADU journey today"
                    description="Expand your income and livable space with a thoughtfully designed ADU. Our team handles everything — from feasibility to final build."
                    primaryLabel="Talk to an ADU Specialist"
                    primaryHref="/talk-to-an-adu-specialist"
                    secondaryText="Or call (909) 500-0917"
                    secondaryHref="tel:+19095000917"
                />
            </main>
            <Footer />
        </>
    )
}
